...
int main (int argc, char **argv){
 ...
 //tries to pipe
    ...
    {   
        ...
        len = strlen(inbuf);
        for (i = 0; i < 1000000; i++) {
            md5((uint8_t*)inbuf, len, result);
        }
        
        char st[16][30];
        // puts the result in string array
        for (i = 0; i < 16; i++){
            sprintf(st[i], "%2.2x", result[i]);
        }
        strcpy(inbuf,"");
        // copy the result to main string;
        for(i=0;i<16;i++){
            strcat(inbuf, st[i]);
        }
        ...
        if(strlen(inbuf)!=32){
            /*
            the child process needs to send 
            a signal that the md5 didn't work
